...
int main(int argc, char *argv[]){
	...
	char buffer[BUFFER_SIZE], server_addr[16] = {'\0'};
	...
	if (sendto(sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&server, sizeof(server)) == SOCKET_ERROR){
		die("Enviar um Datagrama ao Servidor");
	}

	/* aguardar resposta do servidor */
	printf("A aguardar resposta do servidor ...\n");
	if (recvfrom(sock, &response, sizeof(response), 0, (struct sockaddr*)&server, &salen) == SOCKET_ERROR){
	...
	printf("MSG do Casamenteiro { %s }\nA contactar o noivo %s:%d ...\n",
		response.msg, inet_ntoa(response.noivo.sin_addr), response.noivo.sin_port);
