...
int main(int argc, char *argv[])
{
    ...
    while (1){
        ...
        if ((nbytes = recvfrom(sockfd, buffer, sizeof(buffer), 0, (SOCKADDR *) &cli_addr, &sLen)) == SOCKET_ERROR) {
         ...
         *-------------------------------------------------------------------*/
        if ((nbytes = sendto(sockfd, &nbytes, sizeof(nbytes), 0, (struct sockaddr*)&cli_addr, sLen)) == SOCKET_ERROR) {
